...
export type QueryBuilder<T extends Record<string, any>> = (query: {
  query: Query<T>;
}) => string;

const queryBuilder: QueryBuilder<RootQuery> = (query) => {
  return `
    query 
  `;
};

const a = queryBuilder({ query: { heroes: { name: true, id: true } } });
...
export const useFindHeroesQuery = <TData = FindHeroesQuery, TError = unknown>(
  ...
  useQuery<FindHeroesQuery, TError, TData>(
    ...
    fetcher<FindHeroesQuery, FindHeroesQueryVariables>(
      dataSource.endpoint,
